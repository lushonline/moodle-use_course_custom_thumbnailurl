...
class course_summary_exporter extends \core\external\exporter {
    ...
    /**
     * Returns image URL from custom field thumbnailurl
     *
     ...
     * @return null|string Image URL or null if it's not exists.
     */
    private static function get_image_url_from_overview_files(\stdClass $course): ?string {
        $courseinlist = new \core_course_list_element($course);
    ...
    public static function get_course_image($course) {
        ...
        $url = false;
        $url = self::get_image_url_from_overview_files($course);
        if (!$url) {
            $url = self::get_image_url_from_customfield($course);
        }
        return $url;
    }
